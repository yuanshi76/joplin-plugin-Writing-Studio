...
本教程将引导你完成为 Joplin 创建目录插件的步骤。它将在当前笔记旁边显示一个视图，该视图将包含指向笔记各部分的链接。可以点击某个标题以跳转到相关部分。

Through this tutorial you will learn about several aspect of the Joplin API, including:  
通过本教程，你将了解 Joplin API 的多个方面，包括：

- The plugin API  插件 API
...
Before getting any further, make sure your environment is setup correctly as described in the [Get Started guide](https://joplinapp.org/help/api/get_started/plugins).  
在继续之前，请确保您的环境已按照“入门指南”中的说明正确设置。

## Registering the plugin  注册插件[​](#registering-the-plugin "Direct link to Registering the plugin")
...
所有插件都必须注册自己并声明它们可以处理哪些事件。为此，请打开 `src/index.ts` 并按以下方式注册插件。我们还需要在插件启动时运行一些初始化代码，因此也请添加 `onStart()` 事件处理程序：
...
如果您现在构建插件并在 Joplin 中尝试运行它，您应该会在开发控制台中看到消息 `TOC plugin started!` 。

## Getting the current note  获取当前笔记[​](#getting-the-current-note "Direct link to Getting the current note")
...
为了创建目录，您需要访问当前选中的笔记内容，并且每次笔记更改时都需要刷新目录。所有这些都可以使用工作区 API 完成，该 API 提供有关正在编辑的活动内容的信息。

So within the `onStart()` event handler, add the following:  
所以在 `onStart()` 事件处理程序中，添加以下内容：
...
## Getting the note sections and slugs[​](#getting-the-note-sections-and-slugs "Direct link to Getting the note sections and slugs")
...
## Creating a webview[​](#creating-a-webview "Direct link to Creating a webview")
...
## Styling the view[​](#styling-the-view "Direct link to Styling the view")
...
## Making the webview interactive[​](#making-the-webview-interactive "Direct link to Making the webview interactive")
...
## Passing messages between the webview and the plugin[​](#passing-messages-between-the-webview-and-the-plugin "Direct link to Passing messages between the webview and the plugin")
